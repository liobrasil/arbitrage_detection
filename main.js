...
async function processBlockTransactions(blockNumber) {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    if (
      ...
      const blacklistedTokensFound = tokens.filter((token) =>
      ...
      const logData = {
        ...{
          ...
          blacklisted_tokens_symbols: [
            ...new Set(blacklistedTokensFound.map((token) => token.symbol)),
          ],
          from: fromAddress,
          to: toAddress,
          txn_hash: txHash,
          ...
          opportunity_key:
            blockNumber +
            "." +
            venueAddresses.map((addr) => addr.slice(2).toLowerCase()).join("."), //to lower
        },
        ...(botBalance > 0
